            ...
            for industry_concept_code, industry_concept_name in industry_concepts_info.items():
                ...
                for industry_concept_stock_code in get_concept_stocks(industry_concept_code, date=end_date):
                    ...
                    if industry_concept_stock_code in industry_stocks_codes:
                        ...
                        industry_stocks_concepts.iloc[industry_concept_stock_code] = [industry_concept_info[0], industry_concept_info[1]]
    ...
    def get_factors_info_and_data(self):
        ...
        for factor_code in tqdm(factors_codes, desc='calling factors progress'):
            ...
            if factor_code == 'circulating_market_cap':
                factor_data = neutralize(factor_data, how=['jq_l2'], date=self.__end_date, axis=1, fillna=None, add_constant=True)
            else:
                factor_data = neutralize(factor_data, how=['jq_l2', 'ln_circulating_market_cap'], date=self.__end_date, axis=1, fillna=None, add_constant=True)
...
# 定义从开始日期start_date到结束日期end_date，个股列表为stocks_codes，基本面指标所在表对象table的列表为tables的基本面数据字典fundamentals_data取得函数get_all_fundamentals_data
    def get_fundamentals_data(self, tables=[
        ...
        attributes_names_to_delete = ['__module__', '__doc__', '__tablename__', 'id', 'code', 'day', 'pubDate', 'statDate', '__table__', '_sa_class_manager', '__init__', '__mapper__']
        ...
        # 对于宏观指标列表macros中的各宏观因子fundamental
            ...
            # 中性化处理
            fundamental_data = neutralize(fundamental_data, how=['jq_l2', 'ln_circulating_market_cap'], date=self.__end_date, axis=1, fillna=None, add_constant=True)
    ...
    def get_macros_data(self, tables=[
        ...
        def get_attributes_filters(table, attribute_name):
            ...
            if table is macro.MAC_RMB_EXCHANGE_RATE:
                ...
                extra_filter = attribute_name in ['foreign_exchange', 'government_claim', 'bank_claim', 'reserve_money', 'currency_issue']
                ...
                extra_filter = attribute_name in ['social_finance_scale', 'rmb_loan']
        ...
        def get_query_params(table, date):
            ...
            if table is macro.MAC_RMB_EXCHANGE_RATE:
                ...
                if macro_name == 'interest_rate_O/N':
                    term_id = '20'
                elif macro_name == 'interest_rate_1m':
                    term_id = '1'
        ...
        for table in tables:
            ...
            table_and_macros_dict = {attribute_name: {'table': table, 'macro_sql': attribute} 
                                    for attribute_name, attribute in table_attributes_dict.items()
                                    # 筛选其中属性名称attribute_name满足总过滤条件字典filters的属性名称attribute_name: 属性对象attribute，作为宏观名称macro_name: 宏观对象macro字典，返回对应键值对组成的宏观指标所在表对象table的宏观字典table_macros_dict
                                    if get_attributes_filters(table, attribute_name)
                                    }
        ...
        # 对于宏观指标列表macros中的各宏观因子fundamental
            ...
            if macro_name == 'interest_rate_1m':
                # 导出银行间隔月拆借利率，用于填充债券到期收益率的缺失值
                self.__interest_rate_1m_data = macro_data
    ...
    def get_money_flows_data(self):
        ...
        money_flows_merged_codes = money_flows_raw_codes + [money_flow_code
        ...
        for money_flow_code in tqdm(money_flows_merged_codes, desc='calling money flows progress'):
            ...
            if money_flow_code in money_flows_raw_codes:
                ...
                for date in self.dates:
                    ...
                    if money_flow_data.empty:
                        ...
                        money_flow_data['stock_code'] = get_money_flow(security_list=self.__stocks_codes, start_date=date, end_date=date, fields=['sec_code'], count=None)
            ...
            # 中性化处理
            money_flow_data = neutralize(money_flow_data, how=['jq_l2', 'ln_circulating_market_cap'], date=self.__end_date, axis=1, fillna=None, add_constant=True)
    ...
    def get_securities_margins_data(self):
        ...
        securities_margins_used_list = [
            'fin_sec_value',
            'fin_refund_ratio',
            'sec_refund_ratio'
        ]
        
        securities_margins_merged_list = securities_margins_raw_list + [securities_margin 
                                                                        for securities_margin in securities_margins_used_list
        ...
        for securities_margin_name in tqdm(securities_margins_merged_list, desc='calling securities_margins progress'):
            ...
            # 对于构造的因子
            if securities_margin_name in securities_margins_used_list:
                # 构造还子还券比例因子
                if securities_margin_name == 'fin_sec_value':
                        pass
                elif securities_margin_name == 'fin_refund_ratio':
                    securities_margins_data[securities_margin_name] = securities_margins_data.pop('fin_refund_value') / securities_margins_data.pop('fin_buy_value')
                elif securities_margin_name == 'sec_refund_ratio':
                    securities_margins_data[securities_margin_name] = securities_margins_data.pop('sec_refund_value') / securities_margins_data.pop('sec_sell_value')
                ...
                # 中性化处理
                securities_margin_data = neutralize(securities_margin_data, how=['jq_l2', 'ln_circulating_market_cap'], date=self.__end_date, axis=1, fillna=None, add_constant=True)
                # 将各时期dates的fundamentals中的某基本面指标fundamental的基本面数据表fundamental_data作为值添加到各基本面数据表字典fundamentals_data，键设定为基本面指标名称fundamental_name
                securities_margins_data[securities_margin_name] = securities_margin_data
    ...
    def get_billboards_data(self):
        ...
        billboards_used_codes = [
            'amount_ratio',
            'abnormal_code'
        ]
        
        billboards_data = {}
        for billboard_code in billboards_used_codes:
            ...
            if billboard_code == 'abnormal_code':
                ...
                billboards_used_codes += abnormal_codes
            ...
            # 中性化处理
            billboard_data = neutralize(billboard_data, how=['jq_l2', 'ln_circulating_market_cap'], date=self.__end_date, axis=1, fillna=None, add_constant=True)
    ...
    def get_FCF_discounted_model_params_data(self, tables=[
        ...
        def get_query_params(table, date, get_last_day_of_quarter=get_last_day_of_quarter):
            query_params={}
            stock_code_filter = (table.code.in_(self.__stocks_codes))
        ...
        # 单位调整组合
        FCF_discounted_model_params_data['circulating_market_cap'] = FCF_discounted_model_params_data['circulating_market_cap'].copy() * 1e8
        FCF_discounted_model_params_data['interest_bearing_liabilities_interest_rate'] = FCF_discounted_model_params_data['interest_bearing_liabilities_interest_rate'].copy() / 12 / 100

        # 数据组合
        # 现金(Cash):库存现金:银行保险库内持有的各种面值的人民币及外币现金。存放中央银行款项:银行存放于中央银行的款项,包括法定准备金和超额存款准备金。存放同业款项:银行存放于其他银行的款项,通常用于资金清算、头寸调拨等。
        cash = FCF_discounted_model_params_data.pop('cash_equivalents') + FCF_discounted_model_params_data.pop('cash_in_cb') + FCF_discounted_model_params_data.pop('deposit_in_ib')
        ...
        short_investment = FCF_discounted_model_params_data.pop('trading_assets') + FCF_discounted_model_params_data.pop('lend_capital') + FCF_discounted_model_params_data.pop('bought_sellback_assets')
        net_debt = FCF_discounted_model_params_data.pop('interest_bearing_liabilities') - (cash + short_investment)
        enterprise_value = FCF_discounted_model_params_data.pop('circulating_market_cap') + net_debt
        # 计算每个公司的enterprise_value相对于其所在截面的enterprise_value总和的比例，以企业的企业价值在截面中的比重代表其系统重要性
        FCF_discounted_model_params_data['panal_enterprise_value_ratio'] = enterprise_value.apply(lambda panal_enterprise_values: panal_enterprise_values / panal_enterprise_values.sum(), axis=1)
        FCF_discounted_model_params_data['debt_to_enterprise_value_ratio'] = net_debt / enterprise_value
        ...
        FCF_discounted_model_params_data['income_tax_rate'] = FCF_discounted_model_params_data.pop('income_tax_expense') / FCF_discounted_model_params_data.pop('total_profit')

        FCF_discounted_model_params_data['r_wacc'] = (1 - FCF_discounted_model_params_data['debt_to_enterprise_value_ratio']) * self.__returns_data \
        + FCF_discounted_model_params_data['debt_to_enterprise_value_ratio'] * FCF_discounted_model_params_data['interest_bearing_liabilities_interest_rate'] * (1 - FCF_discounted_model_params_data['income_tax_rate'])
