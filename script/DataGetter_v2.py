...
from jqdata import get_all_trade_days, macro, get_money_flow, get_concepts, get_mtss, finance, bond#, get_valuation
...
# 个股信息列表获取
    ...
    def get_securities_margins_data(self):
        ...
        securities_margins_list = [
            ...
            'fin_refund_ratio',  # 融资偿还率
            'sec_refund_ratio'   # 融券偿还率
        ]
        
        # 辅助函数：填充全NaN行
        def fill_nan_rows(df):
            """填充数据框中全为NaN的行，使用上一行有效数据"""
            if df.empty:
                return df
            ...
            for idx in filled_df.index:
                if all_nan_rows[idx]:
                    ...
                    if last_valid_values is not None:
                        # 使用上一行有效值填充
                        filled_df.loc[idx] = last_valid_values
                else:
                    # 更新最后一个有效行
                    last_valid_values = filled_df.loc[idx].copy()

            return filled_df
        
        # 合并列表，确保不重复处理
        securities_margins_merged_list = securities_margins_raw_list + [
            margin for margin in securities_margins_list 
            if margin not in securities_margins_raw_list
        ]
        
        # 处理原始融资融券数据
        for securities_margin_name in tqdm(securities_margins_merged_list, desc='处理融资融券数据'):
            # 处理原始数据字段
            if securities_margin_name in securities_margins_raw_list:
                securities_margin_data_list = []
                for date in self.dates:
                    # 获取融资融券数据
                    securities_margin_data = get_mtss(
                        self.__stocks_codes, 
                        start_date=date, 
                        end_date=date, 
                        fields=['sec_code', securities_margin_name]
                    )
                    ...
                    # 补全可能缺失的股票
                    stocks_codes_df = pd.DataFrame({'stock_code': self.__stocks_codes})
                    securities_margin_data = pd.merge(
                        stocks_codes_df, 
            ...
            if securities_margin_name in securities_margins_list:
                ...
                if securities_margin_name == 'fin_sec_value':
                    ...
                    # 融资融券比例 = 融资买入额 / (融券卖出额 + 融资买入额)
                    numerator = securities_margins_data['fin_buy_value']
                    denominator = securities_margins_data['sec_sell_value'] + securities_margins_data['fin_buy_value']
                    securities_margin_data = numerator / denominator
                    
                elif securities_margin_name == 'fin_refund_ratio':
                    # 融资偿还率 = 融资偿还额 / 融资买入额
                    numerator = securities_margins_data.pop('fin_refund_value')
                    denominator = securities_margins_data.pop('fin_buy_value')
                    securities_margin_data = numerator / denominator
                    
                elif securities_margin_name == 'sec_refund_ratio':
                    # 融券偿还率 = 融券偿还额 / 融券卖出额
                    numerator = securities_margins_data.pop('sec_refund_value')
                    denominator = securities_margins_data.pop('sec_sell_value')
                    securities_margin_data = numerator / denominator
                
                # 数据类型转换和初步清洗
                securities_margin_data = securities_margin_data.astype(float)
                securities_margin_data.replace([np.inf, -np.inf], np.nan, inplace=True)
                
                # 填充全NaN行
                securities_margin_data = fill_nan_rows(securities_margin_data)
                
                # 极端值处理
                securities_margin_data = winsorize_med(
                    securities_margin_data, 
                    scale=5,
                    inclusive=True,
                    inf2nan=False,
                ...
                ## 标准化处理
                #securities_margin_data = standardlize(
                #    securities_margin_data, 
                #    inf2nan=False, 
                #    axis=1
                #)
                ...
                # 存储结果
                if securities_margin_data.any().any():
                    # 存储处理后的融资融券数据
                    securities_margins_data[securities_margin_name] = securities_margin_data
    ...
    def get_industries_data(self):
        ...
        # 初始化行业数据字典，每个行业代码对应一个全0的DataFrame
        jq_l2_industries_data = {
            industry_code: pd.DataFrame(0, columns=self.__stocks_codes, index=self.dates)
            for industry_code in jq_l2_industries_codes
        }

        # 跟踪行业代码变更的映射关系
        industry_code_mapping = {
            'HY07001': 'HY07101',  # 在2021-12-13后的代码变更
            'HY07002': 'HY07102'
        }
        change_date = datetime.date(2021, 12, 13)
        has_change = self.__end_date > change_date  # 判断是否需要处理行业代码变更

        # 预先收集数据，批量处理
        collected_data = {industry_code: [] for industry_code in jq_l2_industries_codes}
        ...
        for date in tqdm(self.dates, desc='industries_data calling progress'):
            ...
            for stock_code, stock_industries_info in date_industries_info.items():
                try:
                    # 获取股票的行业代码
                    if not stock_industries_info.get('jq_l2'):
                        # 如果没有jq_l2行业信息，跳过此股票
                        continue
                    ...
                    # 处理行业代码变更
                    if has_change and date < change_date:
                        if stock_jq_l2_industry_code in industry_code_mapping:
                            stock_jq_l2_industry_code = industry_code_mapping[stock_jq_l2_industry_code]

                    # 标记该股票属于此行业
                    collected_data[stock_jq_l2_industry_code].append((date, stock_code))
        ...
        # 批量更新行业归属关系
        for industry_code, data_points in collected_data.items():
            for date, stock_code in data_points:
                jq_l2_industries_data[industry_code].loc[date, stock_code] = 1

        # 处理完整股数据后进行数据处理（而非循环内多次处理）
        for industry_code, industry_data in jq_l2_industries_data.items():
            # 转换为浮点型，便于后续处理
            industry_data = industry_data.astype(float)

            ## 极端值处理，会导致表中所有值取0
            #industry_data = winsorize_med(industry_data, scale=5, inclusive=True, inf2nan=False, axis=1)

            # 中性化处理
            industry_data = neutralize(
                industry_data, 
                how=['ln_circulating_market_cap'], 
                date=self.__end_date, 
                axis=1, 
                fillna=None, 
                add_constant=True
            ...
            # 更新处理后的数据
            jq_l2_industries_data[industry_code] = industry_data

        # 将未上市或行业信息缺失的股票标记为NaN
        # 这里直接使用pandas的方法，避免循环设置
        for industry_code, industry_data in jq_l2_industries_data.items():
            # 只对有值为0的位置设置为NaN，而不是所有的0都设为NaN
            date_stock_pairs = []
            for date in self.dates:
                date_industries_info = get_industry(security=self.__stocks_codes, date=date)
                for stock_code in self.__stocks_codes:
                    # 如果股票没有行业信息，或者不在市，则需要设置为NaN
                    if (stock_code not in date_industries_info or 
                        'jq_l2' not in date_industries_info.get(stock_code, {}) or
                        industry_data.loc[date, stock_code] == 0):
                        date_stock_pairs.append((date, stock_code))

            # 批量更新为NaN
            if date_stock_pairs:
                for date, stock_code in date_stock_pairs:
                    industry_data.loc[date, stock_code] = np.nan
