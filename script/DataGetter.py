...
# 定义从开始日期start_date到结束日期end_date，个股列表为stocks_codes，基本面指标所在表对象table的列表为tables的基本面数据字典fundamentals_data取得函数get_all_fundamentals_data
    ...
    def get_industries_data(self):
        ...
        for industry_code, data_points in collected_data.items():
            if data_points:  # Only process if we have data points
                # Convert data points to DataFrames for vectorized operations
                dates, stock_codes = zip(*data_points)
                
                # Create a DataFrame with 1s at the specified positions
                industry_data_update = pd.DataFrame(
                    1, 
                    index=dates, 
                    columns=stock_codes
                )
                
                # Use update method to efficiently set values in the original DataFrame
                jq_l2_industries_data[industry_code].update(industry_data_update)
    ...
    def get_FCF_discounted_model_params_data(self, tables=[
        ...
        for FCF_discounted_model_param_name, FCF_discounted_model_param_info in tqdm(tables_and_FCF_discounted_model_params_dict.items(), desc='calling table progress'):
            ...
            if FCF_discounted_model_param_data.any().any():
