...
class CustomRegressor(BaseEstimator, RegressorMixin):
    def __init__(self, random_state, format_df=True,
    dt_params_dist_1={
    ...
    def fit(self, X, y):
        if self.format_df:
            X = self._format_df(X, mode='X')
            y = self._format_df(y, mode='y')
...
class Modeler():
    ...
    def transform_factors_datas_from_dict_to_df(self):
        factors_df = pd.DataFrame(index=self.index)
        ...
        for industry_factors_data in self.industry_factors_datas.values():
            for factor_code, industry_factor_data in industry_factors_data.items():
                factors_df[factor_code] = industry_factor_data
    ...
    def standardize_factors_risks_data(self):
        ...
        factors_codes_missing.extend(
            factor_code
            for factor_code in factors_codes
            if factor_code not in factors_risks_data['factor_code'].values
        )

        # 检查各因子代码是否有重复值
        factors_codes_counts = factors_risks_data['factor_code'].value_counts()
        factors_codes_duplicated = factors_codes_counts[factors_codes_counts > 1].index.tolist()
        ...
        factors_codes_excessive.extend(
            factor_code
    ...
    def visualize_risk_factors_clusters(
        ...
        fig.suptitle('Time Series with Moving Averages by Risk Type')
        ...
        for treatment in risk_components:
            ...
            for outcome in risk_totals:
                ...
                cfdml = CausalForestDML(
                    model_y=CustomRegressor(random_state=self.random_state, format_df=False),
                    model_t=CustomRegressor(random_state=self.random_state, format_df=False),
                    cv=5,  # 交叉验证折数
    ...
    def construct_causal_data(self, treatment, outcome):
    ...
    def visualize_causal_forest_trees(self):
        ...
        fig, axes = plt.subplots(len(risk_totals), len(risk_components), 
                                figsize=(30, 20), dpi=300)

        # First set column titles at the top - BEFORE setting individual subplot titles
        for ax, col_title in zip(axes[0, :], risk_components):
            ax.set_title(col_title, fontsize=16, pad=20)

        # Now set row titles on the left
        for ax, row_title in zip(axes[:, 0], risk_totals):
            ax.set_ylabel(row_title, fontsize=16, rotation=90, labelpad=20)
        ...
        for row_idx, outcome in enumerate(risk_totals):
            for col_idx, treatment in enumerate(risk_components):
                ...
                # Use row_idx to adjust title position differently for first row
                if row_idx == 0:
                    ax.text(0.5, 1.1, f"{treatment} to {outcome}\nTree #{n_estimators}/{n_estimators}",
                        horizontalalignment='center',
                        verticalalignment='bottom',
                        transform=ax.transAxes,
                        fontsize=12)
                else:
                    ax.set_title(f"{treatment} to {outcome}\nTree #{n_estimators}/{n_estimators}")
                
                ax.axis('off')

        # Add overall title with more space above the plot
        plt.suptitle("Causal Forest Last Decision Tree for Each Risk Component to Risk Outcome", 
                    fontsize=20, y=1.08)
    ...
    def visualize_causal_effect(self, treatment_name, outcome_name):
        ...
        model, X, _, T, Y = self.construct_causal_data(treatment_name, outcome_name)
        ...
        ate_result = model.ate(X)
        ate = ate_result.item() if hasattr(ate_result, 'item') else float(ate_result)

        # Get p-value for ATE - properly handle whether it's a method or attribute
        ate_inference = model.ate_inference(X)
        try:
            if callable(getattr(ate_inference, 'pvalue', None)):
                # If pvalue is a method, call it
                p_value_obj = ate_inference.pvalue()
            else:
                # If pvalue is an attribute
                p_value_obj = ate_inference.pvalue
            
            # Convert to proper float
            p_value = p_value_obj.item() if hasattr(p_value_obj, 'item') else float(p_value_obj)
        except Exception:
            p_value = float('nan')  # Use NaN if p-value can't be determined
        ...
        # Plot CATE over time with confidence intervals
        ax1.plot(sorted_time, sorted_effects, linewidth=1.5)
        ax1.scatter(sorted_time, sorted_effects, alpha=0.3, s=10)
        
        try:
            # Get confidence intervals
            effect_inference = model.effect_inference(X)
            if hasattr(effect_inference, 'conf_int'):
                effects_intervals = effect_inference.conf_int()
            else:
                effects_intervals = effect_inference.confidence_intervals_
            
            # Ensure the intervals are 1D arrays
            lower_ci = np.ravel(effects_intervals[0])[sort_idx]
            upper_ci = np.ravel(effects_intervals[1])[sort_idx]
            ax1.fill_between(sorted_time, lower_ci, upper_ci, alpha=0.2, color='blue')
        ...
        try:
            ...
            ax1.plot(x_range, p(x_range), "r--", label="Trend", linewidth=2)
        except Exception as e:
            print(f"Could not plot trend line: {e}")
        
        ax1.set_title(f"Temporal Evolution of {treatment_name} Effect on {outcome_name}", fontsize=14)
        ax1.set_xlabel("Time (Days)", fontsize=12)
        ax1.set_ylabel("Conditional Average Treatment Effect", fontsize=12)
        ax1.axhline(y=0, color='gray', linestyle='--', alpha=0.7)
        ax1.axhline(y=ate, color='red', linestyle='-', alpha=0.7,
        ...
        # ---------- 2. Seasonal Treatment Effects by Month ----------
        ...
        for i, month in enumerate(month_names[1:], 2):  # Start from February (2)
            ...
            if col in X.columns:
                ...
                if np.any(is_month):
                    ...
                    month_errors.append(np.std(effects[is_month]) / np.sqrt(np.sum(is_month)))
                    valid_months.append(month)
        
        # Plot as bar chart with error bars
        x_pos = np.arange(len(valid_months))
        ax2.bar(x_pos, month_effects, yerr=month_errors, align='center', alpha=0.7, 
                color='skyblue', ecolor='black', capsize=10)
        ax2.set_xticks(x_pos)
        ax2.set_xticklabels(valid_months)
        
        ax2.set_title(f"Seasonal Effects of {treatment_name} on {outcome_name}", fontsize=14)
        ax2.set_ylabel('Average Treatment Effect', fontsize=12)
        ax2.set_xlabel('Month', fontsize=12)
        ax2.axhline(y=0, color='gray', linestyle='--', alpha=0.7)
        ax2.axhline(y=ate, color='red', linestyle='-', alpha=0.7, 
                    label=f'ATE: {ate:.4f} (p={p_value:.4f})')
        ax2.grid(alpha=0.3)
        ax2.legend()
        plt.tight_layout()
        ...
        # ---------- 3. Treatment Intensity Analysis ----------
        fig3, ax3 = plt.subplots(figsize=(12, 6))
        
        # Create treatment quintiles
        n_bins = 5
        # Explicitly replace infinity values with NaN
        T_clean = np.array(T)
        T_clean = np.where(np.isfinite(T_clean), T_clean, np.nan)
        
        treatment_bins = pd.qcut(T_clean, n_bins, labels=False, duplicates='drop')
        unique_bins = np.sort(np.unique(treatment_bins[~np.isnan(treatment_bins)]))
        
        bin_labels = ["Very Low", "Low", "Medium", "High", "Very High"]
        if len(unique_bins) < len(bin_labels):
            bin_labels = bin_labels[:len(unique_bins)]
        
        bin_effects = []
        bin_errors = []
        for i, bin_idx in enumerate(unique_bins):
            bin_mask = treatment_bins == bin_idx
            bin_effects.append(np.mean(effects[bin_mask]))
            bin_errors.append(np.std(effects[bin_mask]) / np.sqrt(np.sum(bin_mask)))
        
        # Plot
        x_pos = np.arange(len(bin_labels))
        ax3.bar(x_pos, bin_effects, yerr=bin_errors, align='center', alpha=0.7,
                color='lightgreen', ecolor='black', capsize=10)
        ax3.set_xticks(x_pos)
        ax3.set_xticklabels(bin_labels)
        
        ax3.set_title(f"Effect of {treatment_name} on {outcome_name} by Treatment Intensity", fontsize=14)
        ax3.set_xlabel('Treatment Intensity', fontsize=12)
        ax3.set_ylabel('Average Treatment Effect', fontsize=12)
        ax3.axhline(y=0, color='gray', linestyle='--', alpha=0.7)
        ax3.axhline(y=ate, color='red', linestyle='-', alpha=0.7, 
                    label=f'ATE: {ate:.4f} (p={p_value:.4f})')
        ax3.grid(alpha=0.3)
        ax3.legend()
        plt.tight_layout()
        ...
        sns.histplot(effects_1d, kde=True, ax=ax4, bins=30, color='purple', alpha=0.6)
        ax4.axvline(x=ate, color='red', linestyle='-', linewidth=2, 
                    label=f'ATE: {ate:.4f} (p={p_value:.4f})')
        
        # Add a rug plot to show individual effects
        sns.rugplot(effects_1d, ax=ax4, color='black', alpha=0.3)
        
        ax4.set_title(f"Distribution of Treatment Effects: {treatment_name} on {outcome_name}", fontsize=14)
        ax4.set_xlabel("Effect Magnitude", fontsize=12)
        ...
        # ---------- 5. Treatment vs. Outcome Scatter by Time Period ----------
        fig5, ax5 = plt.subplots(figsize=(12, 6))
        
        # Explicitly replace infinity values with NaN
        Y_clean = np.array(Y)
        Y_clean = np.where(np.isfinite(Y_clean), Y_clean, np.nan)
        ...
        for i, (label, color) in enumerate(zip(q_labels, colors)):
            ...
            # Skip NaN values
            valid_mask = mask & ~np.isnan(T_clean) & ~np.isnan(Y_clean)
            ax5.scatter(T_clean[valid_mask], Y_clean[valid_mask], alpha=0.6, color=color, label=label)
        ...
        for i, (label, color) in enumerate(zip(q_labels, colors)):
            ...
            if np.sum(valid_mask) > 2:  # Need at least 3 points for regression
                z = np.polyfit(T_clean[valid_mask], Y_clean[valid_mask], 1)
                p = np.poly1d(z)
                x_range = np.linspace(np.min(T_clean[valid_mask]), np.max(T_clean[valid_mask]), 20)
                ax5.plot(x_range, p(x_range), color=color, linestyle='--')
        
        ax5.set_title(f"Treatment-Outcome Relationship by Time Period", fontsize=14)
        ax5.set_xlabel(f"{treatment_name} (Treatment)", fontsize=12)
        ax5.set_ylabel(f"{outcome_name} (Outcome)", fontsize=12)
        ax5.grid(alpha=0.3)
        ax5.legend(title="Time Period")
        plt.tight_layout()
        figures.append(fig5)
        
        # ---------- 6. Feature Importance for Heterogeneous Effects ----------
        try:
            ...
            if len(feature_importances) > 0:
                ...
                # Get feature names
                feature_names = X.columns.tolist() if hasattr(X, 'columns') else [f"Feature {i}" for i in range(len(feature_importances))]
                ...
                # Get sorted values
                sorted_importances = valid_features[sort_idx]
                sorted_feature_names = [valid_names[i] for i in sort_idx]
                
                # Plot horizontal bar chart
                ax6.barh(range(len(sorted_importances)), sorted_importances, align='center', color='teal')
                ax6.set_yticks(range(len(sorted_importances)))
                ax6.set_yticklabels(sorted_feature_names)
            
                ax6.set_title("Feature Importance for Effect Heterogeneity", fontsize=14)
...
try:
    ...
    pickle.dump(obj=mol.industry_r_waac_data, file=open(file='../data/modeled_data/bank_factors_datas_train.pkl', mode='wb+'), protocol=4)

# %%
try:
    mol.industry_r_waac_data = pickle.load(open('../data/modeled_data/bank_r_waac_data_train.pkl', mode='rb+'))
except:
    mol.industry_r_waac_data = mol.clean_and_average_r_waac_data()
    pickle.dump(obj=mol.industry_r_waac_data, file=open(file='../data/modeled_data/bank_r_waac_data_train.pkl', mode='wb+'), protocol=4)
...
try:
    ...
    # 创建新图形并保存
    fig = mol.visualize_risk_factors_clusters()
    # 确保保存格式和文件扩展名匹配
